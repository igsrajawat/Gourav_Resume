% Resume of Gourav Singh Rajawat
\documentclass[11pt,a4paper]{article}
\usepackage[margin=0.5in]{geometry} %for page margin setup
\usepackage{multicol}% to divide page in parts to write address and contact information
\usepackage[•]{graphicx} % package used in attaching image with resume
\def\sline{

    \vspace{-5.5em}
    \hspace{\fill}\line(1,0){500}\hspace{\fill}
}%A single line under title

\begin{document}
\title{Gourav Singh Rajawat}% giving title to document
\author{}
\date{}
\maketitle
\sline  %sline call
\begin{multicols}{2} %multicol package start
\flushleft{ %this part is shown on left of the page
\large{ %larger words than normal
8/12 B, Khandu Colony\\
Banswara,Rajasthan\\
Pin:327001\\
}
}
\columnbreak % now from here content will be go to next part
\flushright{ % content shown on right part of page
\large{  
Contact:9501179788\\
\texttt{gsrajawat1997@gmail.com}\\ % type of font i.etypewriter font
\includegraphics[width=4cm, height=4cm]{img.jpg}\\ %including image since no path is describe so it tries to find image from path where the latex file is
}
}
\end{multicols} %multicol ends here

\section*{Education} % A new section education added
\vspace{-0.7em}\hline \vspace{1.5em} %for a single line
	\begin{tabular}{|c|c|c|c|c|} % to create a table
	\hline %single line
	...
	\end{tabular}	%table ends
	
	
\section*{Project}
\vspace{-0.7em}\hline \vspace{1.5em}
\begin{enumerate} %enumerate i.e points shown by number begin
...
\section*{Competition}
\vspace{-0.7em}\hline \vspace{1.5em}
\begin{itemize} %itemize i.e points shown by bulletin 
